...
int main(int argc, char *argv[]){
    ...
    SDL_Surface *lignes[510] = {NULL}; /*tableau de pointeurs qui va stocker la surface d'une ligne dans l'écran
    et ceci 256 fois, chaque ligne est à NULL*/
    SDL_Surface *lignes2[510] = {NULL};
    int i = 0, j = 0;

    SDL_Rect position; //variable de type SDL_Rect, c'est une structure qui contient des sous-variables servant aux coordonnées

    ecran =  SDL_SetVideoMode(640,512,32, SDL_HWSURFACE);//On tente d'ouvrir une fenêtre, 512 de hauteur pour correspondre aux lignes
...
//La SDL est chargée. On peut y mettre le contenu du programme
    ...
    for(i = 0; i < 510; i++){
        lignes2[i] = SDL_CreateRGBSurface(SDL_HWSURFACE,640,1,32,0,0,0,0);/*On alloue à chaque ligne du tableau
        une largeur de 640 et 1 de hauteur, en 32 bits, les autres paramètres ne nous interesse pas*/
    }


for(i = 0; i < 256; i++){
       position.x = 0;//Les lignes commence toute à gauche (absicce à 0)
...
for(i = 256; i < 510; i++){
     ...
     SDL_FillRect(lignes2[i],NULL,SDL_MapRGB(ecran->format,j,j,j));/*remplissage de la ligne avec une couleur, deuxième paramètre
     à null pour indiquer qu'on prend toute la surface de la ligne*/
         SDL_BlitSurface(lignes2[i],NULL,ecran,&position); /*On blitte (coller) notre surface rectangle sur l'écran,
         le deuxième paramètre est à null pour indiquer la taille de la surface à coller, comme on prend tous il faut mettre null
         ,il faut envoyer l'adresse de variable position*/
         j = j - 1;
}
...
for(i = 0; i < 510; i++){
    SDL_FreeSurface(lignes2[i]);//Libération de la surface
}
